          ...
          <q-card-section style="flex-grow: 1; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <div class="text-subtitle2 text-grey q-mb-sm">
                <b>{{ item.uraian_bidang_csr_force }}</b>
          ...
          <div>
            ...
            <q-list dense bordered separator class="rounded-borders">
              <q-item>
                ...
                <q-item-section>{{ selectedItem.uraian_bidang_csr_force }}</q-item-section>
              </q-item>
            ...
            <q-list dense bordered separator class="rounded-borders">
              <q-item>
                ...
                <q-item-section>
                  ...
                  <q-badge v-else-if="selectedItem.status == 4" color="green-6" text-color="white"
                    label="Selesai" />
                </q-item-section>
              </q-item>
              <q-item>
      ...
      fetch(this.$store.state.url.FORCEMAJEURE + "kecamatan", {
        ...
        .then(res_data => {
          this.list_kecamatan = res_data.map(k => ({
            ...k,
            kecamatan_id: String(k.kecamatan_id) // 🔑 paksa string
          }));
...
for (let key in this.form) {
  if ((key === "file" || key === "file_spec") && this.form[key]) {
    ...
    editData() {
      ...
      fetch(this.$store.state.url.FORCEMAJEURE + "editData", {
        method: "POST",
        headers: {
          authorization: "kikensbatara " + localStorage.token
        },
    ...
    async openEditModal(item) {
      ...
      this.form = {
        ...this.form,                
        id: item.id,                 
        bidang_force_id: item.bidang_force_id ? String(item.bidang_force_id) : null,
        ...
        desa_id: item.desa_id ? String(item.desa_id) : null,
        alamat: item.alamat || "",
        file: null,                
    ...
    getDesaEdit(kecamatan_id, presetDesaId) {
      ...
      return fetch(this.$store.state.url.FORCEMAJEURE + "desa", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token
